...
namespace facade {
glm::mat4x4 Camera::view(Transform const& transform) const {
...
glm::mat4x4 Camera::projection(glm::vec2 extent) const {
	if (extent.x == 0.0f || extent.y == 0.0f) { return glm::identity<glm::mat4x4>(); }
