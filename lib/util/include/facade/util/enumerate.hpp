...
struct Enumerated {
	...
	constexpr Type get() const { return t; }
};
...
class Enumerator {
  public:
	...
	struct iterator {
		...
		reference operator*() const { return {*it, index}; }
		iterator& operator++() { return (++it, ++index, *this); }
	...
	constexpr iterator begin() const { return {m_begin, static_cast<Index>(0)}; }
